...
function Editproduct() {
    ...
    let NameCategory = getCategories.find(el => el._id === Product?.productCategory)
    return (
                ...
                <div className="grid grid-cols-1 gap-4 mb-4">
                    ...
                    <div className="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 mt-16">
                        ...
                        <form class="max-w-sm mx-auto " onSubmit={EditProduct}>
                            <div class="mb-5 mt-5">
                                <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"> Text kiriting!</label>
                                <p className='text-sm text-gray-800 dark:text-gray-200 mb-2'>
                                    {Product.productText}
                                </p>
                                <textarea placeholder='yangi textni kiriting...' value={productText} onChange={e => setProductText(e.target.value)} id="message" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" ></textarea>
                            </div>
                            <div class="mb-5">
                                <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Variantlar</label>
                                <div className="flex flex-col">
                                    {
                                        Product.productOptions.map(option => (
                                            <span className=' border py-2 px-3 rounded-lg mb-2 text-sm font-medium text-gray-900 dark:text-white'>{option}</span>
                                        ))
                                    }
                            ...
                            <div class="mb-5">
                                ...
                                <input value={productAnswer} onChange={e => setProductAnswer(e.target.value)} type="text" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="yangi javobni kiriting..." />
                            </div>

                            <div class="mb-5">
                                <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Kategoriya</label>
                                <p className='border py-2 px-3 rounded-lg mb-2 text-sm font-medium text-gray-900 dark:text-white'>
                                    {NameCategory?.categoryName}
                                </p>
                                <select onChange={e => setProductCaterory(e.target.value)} id="countries" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            ...
                            <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">O`zgartirish</button>
                        </form>
